...
ggplot(data = crimes_data_filtered) +
  aes(District) + 
  facet_wrap(~ Type) +
  geom_bar()
```
...
crime_counts <- crime_counts %>%
  mutate(radius = sqrt(cumsum_count/pi))

# plot
ggplot(crime_counts, aes(x = 1, y = cumsum_count, size = radius, fill = `Primary Type`)) +
  geom_point(shape = 21, color = "black") +
  scale_size_continuous(range = c(5, 20)) +
  theme_void() +
  theme(legend.position = "right") +
  labs(title = "Packed Circle Chart of Crime Types",
       x = NULL, y = NULL)
```
...
## Graph 6
This is a heatmap to visualize the concentration of crimes by week of the month and day of the month.
The color intensity in the heatmap represents the number of crimes. 
```{r}
library(lubridate)
library(dplyr)

crimes_data$Date <- as.Date(crimes_data$Date, "%Y-%m-%d") 
crimes_data$DayOfMonth <- day(crimes_data$Date)
crimes_data$WeekOfMonth <- ceiling(day(crimes_data$Date) / 7)

crimes_data <- crimes_data %>%
  group_by(DayOfMonth, WeekOfMonth) %>%
  summarise(CrimeCount = n())

# heatmap
ggplot(crimes_data, aes(x = WeekOfMonth, y = DayOfMonth, fill = CrimeCount)) +
  geom_tile(colour = "white") +
  scale_fill_gradient(low = "white", high = "steelblue") +
  theme_minimal() +
  labs(title = "Heatmap of Crime Concentration",
       x = "Week of the Month",
       y = "Day of the Month",

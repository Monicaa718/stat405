...
#| context = "server"
  ...
  draw_circle <- function(row, col, size, color) {
  pushViewport(viewport(layout.pos.row = row+1, layout.pos.col = col))
  # Use a fixed size for now to ensure circles are drawn
  grid.circle(gp = gpar(fill = color, alpha = 0.5), r = unit(0.1, "npc")) # Fixed size for testing
  popViewport()
}
...
calculate_size_color <- function(district, type, year, month) {
  ...
  if (nrow(district_data) == 0) {
    return(c(0, "grey"))  # Default values if no data is available
  }
  type_district_data <- filter(district_data, `Primary Type` == type)
  if (nrow(type_district_data) == 0) {
    return(c(0, "grey"))  # Default values if no data is available for the type
  }
  type_count <- as.numeric(type_district_data$Num_Crimes)
  total_type_count <- sum(as.numeric(district_data$Num_Crimes))
  size <- if (total_type_count > 0) (type_count / total_type_count) else 0
  color <- get_color(as.numeric(type_district_data$Arrest_Rate))
  return(c(size, color))
}
...
output$CrimesPlot <- renderPlot({
  ...
  grid.newpage()
    ...
    colu <- 1
    for (row in 2:6) {
      # Draw row label (only once per row, when in the first column)
      pushViewport(viewport(layout.pos.row = row, layout.pos.col = colu))
      grid.text(row_labels[row-1], x = 0.1, just = "left")  # Adjust 'x' and 'just' as needed
      popViewport()
    }
    ...
    # for (col in 2:6) {
    # 
    #   district <- toString(switch(col-1, input$zipcode1, input$zipcode2, input$zipcode3, input$zipcode4, input$zipcode5))
    # 
    #   pushViewport(viewport(layout.pos.row = 1, layout.pos.col = col))
    #   grid.text(district, y = 0.9, just = "top")  # Adjust 'y' and 'just' as needed
    #   popViewport()
    # 
    # 
    #   for (row in 1:3) {
    #     size <- calculate_size_color(district, row_labels[col-1], "year", "month")[1]
    #     color <- calculate_size_color(district, row_labels[col-1], "year", "month")[2]
    #     draw_circle(row, col, size, color)
    #   }
    # }
    ...
    for (row in 1:3) {
      size_color <- calculate_size_color(district, row_labels[col-1], input$year, input$month)
      draw_circle(row, col, size_color[1], size_color[2])
    }
  }

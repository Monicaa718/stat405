...
draw_circle <- function(row, col, size, color) {
  ...
  scaled_size <- max(size * 10, 0.05)  # Adjust the scaling factor as needed
  pushViewport(viewport(layout.pos.row = row+1, layout.pos.col = col))
...
calculate_size_color <- function(district, type, year, month) {
  ...
  print(paste("Color for", type, "in district", district, ":", color))
...
for (col in 2:6) {
  ...
  for (row in 1:3) {
    ...
    draw_circle(row, col, size_color$size, size_color$color)
  }
}
